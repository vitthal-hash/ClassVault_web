...
import { Notes } from "@/lib/local/repo";
...
export function NotesTab({ subjectId }: { subjectId: number }) {
  ...
  const load = useCallback(async () => {
    if (!username) return;
    setItems(await Notes.forSubject(username, subjectId));
  }, [username, subjectId]);
  ...
  const create = async () => {
    ...
    await Notes.create(username, subjectId, title.trim() || null, body.trim());
    setTitle("");
    setBody("");
    setShowForm(false);
    load();
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <div className="card p-10 text-center text-sm text-muted">No notes yet.</div>
      ) : (
